...
# ✅ STARTUP PERFORMANCE FIX: Remove heavy imports from module level
# Heavy libraries (numpy, nltk, sklearn, etc.) are now imported lazily when needed

# Initialize logger first before any other code that might use it
logger = logging.getLogger(__name__)

# ✅ STARTUP PERFORMANCE FIX: Lazy initialization flags instead of eager imports
_nltk_available = None
_pdf_support_available = None
_docx_support_available = None
_sklearn_available = None
_langdetect_available = None

def _get_nltk_components():
    """🚀 LAZY: Get NLTK components only when needed"""
    global _nltk_available
    
    if _nltk_available is not None:
        return _nltk_available  # Return cached result
    ...
    try:
        ...
        # Test NLTK data availability and download if needed
        try:
...
def _get_pdf_support():
    """🚀 LAZY: Get PDF support only when needed"""
    global _pdf_support_available
    
    if _pdf_support_available is not None:
        return _pdf_support_available
        
    try:
        import fitz  # PyMuPDF
        _pdf_support_available = fitz
        logger.debug("✅ PDF support loaded")
        return fitz
    except ImportError:
        logger.warning("PyMuPDF not available. PDF processing will be limited.")
        _pdf_support_available = False
        return False

def _get_docx_support():
    """🚀 LAZY: Get DOCX support only when needed"""
    global _docx_support_available
    
    if _docx_support_available is not None:
        return _docx_support_available
        
    try:
        from docx import Document
        _docx_support_available = Document
        logger.debug("✅ DOCX support loaded")
        return Document
    except ImportError:
        logger.warning("python-docx not available. DOCX processing will be limited.")
        _docx_support_available = False
        return False

def _get_sklearn_components():
    """🚀 LAZY: Get scikit-learn components only when needed"""
...
def _get_langdetect():
    """🚀 LAZY: Get language detection only when needed"""
    ...
    try:
        import langdetect
        _langdetect_available = langdetect
...
def _get_numpy():
    """🚀 LAZY: Get numpy only when needed"""
    try:
        import numpy as np
        return np
    except ImportError:
...
class AdvancedDocumentProcessor:
    ...
    def __init__(self, preserve_educational_content: bool = True, use_semantic_chunking: bool = True):
        ...
        # 🚀 STARTUP PERFORMANCE FIX: Defer heavy initialization until needed
    ...
    def _ensure_components_initialized(self):
        """🚀 LAZY: Initialize heavy components only when first needed"""
        if self._components_initialized:
            return
            
        # Initialize NLTK components if needed and not preserving educational content
        if not self.preserve_educational_content:
            nltk_components = _get_nltk_components()
            if nltk_components:
                try:
                    self.lemmatizer = nltk_components['WordNetLemmatizer']()
                    self.stop_words = set(nltk_components['stopwords'].words("english"))
                except Exception as e:
        ...
        if self.use_semantic_chunking:
            ...
            if sklearn_components:
                self.tfidf_vectorizer = sklearn_components['TfidfVectorizer'](
                    max_features=1000,
                    stop_words='english' if not self.preserve_educational_content else None,
                    ngram_range=(1, 2)
                )
            else:
                self.tfidf_vectorizer = None
        ...
        logger.debug("✅ Document processor components initialized")
    ...
    def extract_text_with_structure(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_path.suffix.lower() == ".pdf":
                fitz = _get_pdf_support()
                if fitz:
                    content, meta, structural_info = self._extract_from_pdf_enhanced(file_path, fitz)
                    if meta and isinstance(meta, dict):
                        metadata.update(meta)
                else:
                    raise ValueError("PDF support not available")
            elif file_path.suffix.lower() == ".docx":
                Document = _get_docx_support()
                if Document:
                    content, meta, structural_info = self._extract_from_docx_enhanced(file_path, Document)
                    if meta and isinstance(meta, dict):
                        metadata.update(meta)
                else:
                    raise ValueError("DOCX support not available")
            elif file_path.suffix.lower() == ".txt":
                content = self._extract_from_txt_enhanced(file_path)
    ...
    def _extract_from_pdf_enhanced(self, file_path: Path, fitz) -> Tuple[str, Dict[str, Any], Dict[str, Any]]:
        """🚀 ENHANCED: Extract PDF with structure preservation"""
        doc = None
        try:
    ...
    def _extract_from_docx_enhanced(self, file_path: Path, Document) -> Tuple[str, Dict[str, Any], Dict[str, Any]]:
    ...
    def _extract_from_txt_enhanced(self, file_path: Path) -> str:
        """🚀 ENHANCED: Extract TXT with better encoding handling"""
        try:
    ...
    def semantic_chunking(self, text: str, max_chunk_size: int = 500, overlap_sentences: int = 2) -> List[Dict[str, Any]]:
        """🚀 ADVANCED: Semantic chunking with lazy loading of sklearn components"""
        self._ensure_components_initialized()  # Initialize heavy components only when needed
        ...
        nltk_components = _get_nltk_components()
        if nltk_components and nltk_components.get('sent_tokenize'):
            try:
                sentences = nltk_components['sent_tokenize'](text)
            except Exception as e:
        ...
        if self.tfidf_vectorizer and len(sentences) > 1:
            try:
                return self._embedding_based_chunking(sentences, max_chunk_size, overlap_sentences)
            except Exception as e:
                logger.warning(f"Semantic chunking failed: {e}, falling back to sentence window")

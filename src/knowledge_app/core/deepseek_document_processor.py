...
# from .deepseek_integration import get_deepseek_pipeline, DeepSeekConfig  # Removed broken import
...
class DeepSeekDocumentProcessor:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Skip DeepSeek pipeline initialization - we'll use direct API calls
    ...
    async def _process_chunk_with_deepseek(self, chunk: str, progress_callback: Optional[Callable] = None) -> Optional[Dict[str, Any]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Use the document processing method from DeepSeek pipeline
                result = self.deepseek_pipeline.process_document_content(
                    content=chunk,
                    processing_type="training_data",
    ...
    def _analyze_result_deficiencies(self, result: Dict[str, Any]) -> List[str]:
        ...
        try:
            # CRITICAL FIX: Input validation to prevent KeyError and ensure safe operations
            if not isinstance(result, dict):
                logger.warning("⚠️ Result is not a dictionary, returning general improvement")
            ...
            # CRITICAL FIX: Validate that retrieved values are lists to prevent errors
            if not isinstance(concepts, list):
                concepts = []
            if not isinstance(training_data, list):
                training_data = []
            ...
            # CRITICAL FIX: Check for division by zero - ensure config values are positive
            min_concepts = max(1, getattr(self.config, 'min_concepts_per_chunk', 2))
            min_training = max(1, getattr(self.config, 'min_training_examples_per_chunk', 3))

            # CRITICAL FIX: Ensure we have valid configuration values to prevent division by zero
            if min_concepts <= 0:
                min_concepts = 2  # Safe default
            if min_training <= 0:
                min_training = 3  # Safe default

            # Check concept quality with safe comparison
            if len(concepts) < min_concepts:
                deficiencies.append("insufficient_concepts")

            # Check training data quality with safe comparison
            if len(training_data) < min_training:
            ...
            if getattr(self.config, 'phd_level_enforcement', False):
                ...
                # CRITICAL FIX: Safe iteration to prevent errors
                if training_data:  # Only check if training_data is not empty
                    try:
                        has_phd_content = any(
                            any(indicator in str(item).lower() for indicator in phd_indicators)
                            for item in training_data if item and isinstance(item, (dict, str))  # Skip None/invalid items
                        )
                    except (AttributeError, TypeError) as e:
                        logger.warning(f"⚠️ Error checking PhD content: {e}")
                        has_phd_content = False
            ...
            # CRITICAL FIX: Ensure we always return a valid list
            if not deficiencies:
                deficiencies.append("general_improvement")
